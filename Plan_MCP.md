...
The repository currently contains the architecture documents but no MCP source, Python package scaffold, or curated-paper JSON. Create the implementation under the shared `src/flowstate/` package defined in Plan_Workflow.md; keep the human-owned seed file at `data/research/curated_papers.json`.
...
4. Organize code by behavior:
   - `src/flowstate/knowledge/models.py` — Pydantic input/output and stored-record models.
   - `src/flowstate/knowledge/store.py` and `schema.sql` — transactions, append-only ingestion events, FTS, vectors, and migrations.
   - `src/flowstate/knowledge/providers/openalex.py`, `github.py`, and `papers_with_code.py` — replaceable read-only adapters.
   - `src/flowstate/knowledge/ingestion.py` — curated, scheduled, and on-demand ingestion through one queue.
   - `src/flowstate/knowledge/sanitize.py` — text normalization, prompt-injection flags, and quarantine decisions.
   - `src/flowstate/knowledge/retrieval.py` — hybrid ranking and evidence assembly.
   - `src/flowstate/knowledge/cache.py` and `budgets.py` — canonical query cache, request/document/token caps, rate limits, and bounded backoff.
   - `src/flowstate/mcp/server.py` — the only MCP-facing tool definitions.
   - `src/flowstate/cli/knowledge.py` — human-only ingestion, validation, status, and reindex commands.
...
3. Expose `python -m flowstate.cli knowledge ingest-scheduled --once`. The main workflow scheduler may invoke it weekly; do not require a permanently running cron service. A repeated command must resume expired leases and never duplicate a stored paper.
...
- Hugging Face Papers discovery is a public, unauthenticated endpoint; no key or mailto is required.
