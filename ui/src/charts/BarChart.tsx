...
export function BarChart({ title, xAxisLabel, yAxisLabel, categories, series, onBarClick }: BarChartProps) {
  ...
  useEffect(() => {
    ...
    chart.setOption({
      ...
      series: series.map((entry) => ({ name: entry.name, type: "bar", data: entry.values })),
    });
    if (onBarClick) {
  ...
  }, [categories, series, xAxisLabel, yAxisLabel, reducedMotion, hasData, onBarClick]);
