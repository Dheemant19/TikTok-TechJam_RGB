...
class KnowledgeStore:
    ...
    def papers_missing_verified_code(self, paper_ids: list[str]) -> set[str]:
        """Return which of the given paper_ids have zero verified (pinned +
        licensed) repository on record. Used to skip re-resolving curated
        papers' GitHub repositories once every one of them already has a
        verified record, so `ensure_curated_bank` costs nothing on repeat
        session starts."""
        if not paper_ids:
            return set()
        ...
        with self.connect() as connection:
            rows = connection.execute(
                f"""SELECT DISTINCT paper_id FROM code_implementations
                    WHERE paper_id IN ({placeholders})
                    AND pinned_commit IS NOT NULL AND license IS NOT NULL""",
                paper_ids,
            ).fetchall()
        verified = {row["paper_id"] for row in rows}
        return set(paper_ids) - verified
