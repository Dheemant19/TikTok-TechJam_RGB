...
class MCPApplication:
    ...
    def _register_tools(self) -> None:
        ...
        @self.mcp.tool()
        async def discover_paper_with_code(query: str) -> CodeForPaperResult:
            """Fetch exactly one fresh Hugging Face paper with a linked GitHub
            repository for `query`; hard-checked -- a paper with no linked
            repository is never returned -- and never a repeat of a paper
            already surfaced this session."""
            return await self._recorded(
                "discover_paper_with_code", query,
                lambda: self.runtime.retrieval.discover_code_paper(query),
            )
...
def get_application() -> MCPApplication:
    ...
    if _application is None:
        _application = create_application(os.getenv("FLOWSTATE_KNOWLEDGE_CONFIG", "configs/knowledge/research.yaml"))
    return _application
...
def main() -> None:
    transport = os.getenv("FLOWSTATE_MCP_TRANSPORT", "stdio")
    if transport not in {"stdio", "streamable-http"}:
        raise SystemExit("FLOWSTATE_MCP_TRANSPORT must be stdio or streamable-http")
