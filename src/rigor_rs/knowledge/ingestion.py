...
from rigor_rs.knowledge.embeddings import EmbeddingProvider, vector_to_bytes
...
from rigor_rs.knowledge.store import KnowledgeStore, utc_now
...
class IngestionService:
    ...
    async def _process_curated(
        ...
        if enrich and self.openalex:
            identifier = next((value for _, value in source.identifiers.stable_values()), None)
            if identifier:
                enriched = await self.openalex.get_work(identifier)
        title = source.title
    ...
    async def ingest_scheduled(self, once: bool = True) -> list[IngestionReceipt]:
        if not self.openalex:
            return []
        discovered = 0
        for area in self.config.ingestion.scheduled_priority_areas:
            if discovered >= self.config.ingestion.maximum_automatic_documents:
                break
            result = await self.openalex.search(area.replace("_", " "), None, min(8, self.config.ingestion.maximum_automatic_documents - discovered))
            if result.error:
                continue
            await self.enqueue_provider_records(result.records, area)
            discovered += len(result.records)
        return await self.process_queue(resolve_code=False)
