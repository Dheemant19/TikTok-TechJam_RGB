...
auditable, evidence-driven research loop.
...
1. KuaiRand-Pure is the required primary benchmark. KuaiRand-1k and
   KuaiRand-27k remain disabled until KuaiRand-Pure is submission-ready.
...
8. KuaiRand-Pure reports NDCG@10 and Recall@50, with click as the fixed
   positive relevance label. Both contribute equally to the primary composite
   delta.
...
1. Pin the organizer-provided KuaiRand baseline revision and exact environment.
...
- Counts by split and by user (for per-user ranked-list evaluation).
- Click-positive-rate prevalence per split (click is the fixed positive
  relevance label).
...
optimizer/regularization changes, multi-task objectives across the 12
KuaiRand feedback signals (click, like, follow, long_view, …), task-sharing
changes, MMoE/PLE for measured negative transfer, and interaction models such
as DeepFM/DCN. A fashionable architecture is not itself evidence.
...
expected_outcomes:
  ...
  task_tradeoff:
    signature: ndcg_up_recall_down_or_reverse
...
7. Run data-schema, split-taint, leakage, evaluator, and submission-schema
   contracts.
8. Run metamorphic metric tests, including row-order invariance and per-user
   ranked-list scoring behavior.
...
For KuaiRand-Pure validation:

```text
delta_ndcg = R[NDCG@10, validation] - B[NDCG@10, validation]
delta_recall = R[Recall@50, validation] - B[Recall@50, validation]
composite_delta_vs_baseline = (delta_ndcg + delta_recall) / 2
```

The same formula is computed against the selected parent. Absolute difference,
not relative percentage, is used.

Diagnostics may include per-user ranking-score distribution, log loss on the
click-positive label, calibration error, auxiliary-task losses (when the 12
KuaiRand feedback signals are used for multi-task training), training/validation
gap, learning curves, subgroup behavior, cold/unseen-ID behavior, runtime,
memory, and task-gradient conflict when auxiliary signals are used. Precision
at fixed thresholds is a diagnostic only and is never confused with the primary
NDCG@10/Recall@50 objective.
...
| NDCG@10 improves while Recall@50 worsens, or reverse                     | Negative transfer evidence                     | Test loss balance, auxiliary-task weighting, expert routing, or ranking-objective consistency one at a time |
...
| Empty candidate set or user with no positive (clicked) items | Label/subset validation | Correct split/config if wrong; otherwise mark metric undefined for that user per evaluator spec | Result invalid and non-comparable                        |
...
| NDCG/Recall trade-off                 | Metric/loss/gradient evidence        | Pre-registered task-balancing or sharing ablation                                            | Promote only under composite/guardrail policy            |
...
benchmark: kuairand_pure | kuairand_1k | kuairand_27k
...
8. Near-noise seed confirmation occurs only when its information value justifies
   the cost.
9. KuaiRand-1k/27k consume no primary budget until KuaiRand-Pure is finalized.
...
the primary click-ranking objective and any auxiliary KuaiRand feedback
signals used for multi-task training (like, follow, long_view, comment, …).
Use measured interference to justify loss reweighting, tower separation,
MMoE, or PLE. This connects architecture choice directly to auxiliary-signal
evidence.
...
- Shuffled labels move NDCG@10/Recall@50 toward chance in a controlled test
  fixture.
- NDCG@10/Recall@50 are computed per-user, over exactly the candidate set the
  evaluation script defines.
...
configs/
  challenge/
    kuairand_pure.yaml
    kuairand_1k.yaml               # disabled until KuaiRand-Pure-ready gate
    kuairand_27k.yaml              # disabled until KuaiRand-Pure-ready gate
  baseline/
  experiments/
  budgets/
data/
...
4. **Metric view**: NDCG@10, Recall@50, composite deltas versus parent and
   official baseline, and uncertainty status.
...
## 20. KuaiRand-1k / KuaiRand-27k bonus gate

KuaiRand-1k and KuaiRand-27k are separate benchmark namespaces with separate
manifests, run frontier, and artifacts, using the same NDCG@10/Recall@50
metric plugin as KuaiRand-Pure at larger scale. They are enabled only when all
of the following are true:

1. KuaiRand-Pure official baseline is reproduced.
2. KuaiRand-Pure has a reliable end-to-end run.
3. KuaiRand-Pure validation-best artifact is preserved and replayable.
4. KuaiRand-Pure final packaging passes.
5. A separate bonus budget remains.

KuaiRand-1k and KuaiRand-27k use organizer-supplied train/validation/test
definitions and report the same NDCG@10 and Recall@50 metrics as
KuaiRand-Pure. Their randomized exposure data may support advanced off-policy
research, but no bonus-dataset work may jeopardize the KuaiRand-Pure primary
result.
...
- [ ] Raw data manifest, split integrity, label definitions, and leakage checks
      pass.
- [ ] Official KuaiRand-Pure baseline was reproduced and has an evaluator
      receipt.
...
- [ ] NDCG@10 and Recall@50 are both reported.
...
- [ ] KuaiRand-1k/27k claims exist only if genuinely attempted after the
      KuaiRand-Pure gate.
